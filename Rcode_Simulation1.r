...
C_test=1*as.numeric(1*testX[,7]^2+1.5*testX[,8]^2+2*testX[,9]+1.5*testX[,10])

g_test=as.numeric(I(C_test>0))



V_true=mean(exp(2+ as.numeric(testX%*%gamma)))
...
V_true=mean(exp(2+ as.numeric(X%*%gamma)-abs(1+1.5*X[,1]-2*X[,2])*(A_opt-g_test)^2))
